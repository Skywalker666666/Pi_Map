...
#include "cartographer_ros/include/msg_utils/pi_msg_adaptor.h"
...
class Node {
 public:
  ...
  void SavePoseToFiles();
  
  void RunMapSave(const std::string& pbstream_filename, const std::string& map_filestem,
 ...
 private:
  ...
  std::shared_ptr<PIAUTO::msg::PIMsgAdaptor> msg_adp;
  int pub_handler;
  int node_id_in_local_ = 0;

  //added by skywalker

  int pose_tx_counter_ = 0;
  int pose_publisher_tx_counter_ = 0;
  
  struct PoseOutput {
    // index for visualization
    int pose_tx_index;
    // ToDo: how to use this time in offline mode?
    ::cartographer::common::Time time;
    // the newest node id, if the scan killed by motion filter,
    // node id is the last successful one.
    int node_id_from_local;
    // pose in Rigid3d format
    ::cartographer::transform::Rigid3d x_pose;
    // tf interpolated version, tx counter for those published for sonar
    // used for post processing, easy for debug
    int pose_publisher_tx_index;
  };
  
  // local pose from call back, for point cloud display
  // it is from local scan match.
  std::vector<PoseOutput> original_local_poses_;
  // generated from global optimal transform. bar(MLSUBMx) * inv(MLSUBMx) * local_pose
  std::vector<PoseOutput> optimized_global_poses_;
  
  std::vector<PoseOutput> loctomap_adjust_poses_;
};
