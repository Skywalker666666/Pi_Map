...
DEFINE_string(pbstream_filename_comb, "",
    "save pbstream combined with node.");

DEFINE_string(map_filestem_comb, "",
    "save map file combined with node.");

DEFINE_double(resolution_comb, 0.05,
...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory) {
  ...
  //added by Skywalker  
  /*for (auto& original_local_pose : node.original_local_poses_)
  {
    std::cout << original_local_pose.time << std::endl;
    std::cout << original_local_pose.node_id_from_local << std::endl;
    std::cout << original_local_pose.x_pose << std::endl;     
  }*/
  node.SavePoseToFiles();
  
  // To Do, this offline_node is not used now, and pbstream file is not generated.
  // Only after pbstream file is generated can we read it in by following line.
  
  node.RunMapSave(FLAGS_pbstream_filename_comb, FLAGS_map_filestem_comb,
                  FLAGS_resolution_comb);
  
  ////Following method doesn't work, 
  ////Here, we have killed the cartographer_node in ROS
  //std::string AnyKeys;
  //std::cout << "If map is saved, press AnyKeys to go on!!!";
