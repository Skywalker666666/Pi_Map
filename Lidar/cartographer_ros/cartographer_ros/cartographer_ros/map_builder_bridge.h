...
class MapBuilderBridge {
 public:
  ...
  //added by skywalker, for node id in come with local data.
  int GetNodeId(const int trajectory_id);
  
  //using MapById = ::cartographer::mapping::MapById;
  using NodeId = ::cartographer::mapping::NodeId;
  using TrajectoryNodePose = ::cartographer::mapping::TrajectoryNodePose;
  
  ::cartographer::mapping::MapById<NodeId, TrajectoryNodePose> GetNodeListForFusion();
 ...
 private:
  ...
  // added by skywalker, for node id of local pose from callback.
  std::unordered_map<int, int> local_node_id_;
