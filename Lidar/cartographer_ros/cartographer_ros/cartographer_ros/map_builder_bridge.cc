...
void MapBuilderBridge::OnLocalSlamResult(
  ...
  if (insertion_result != nullptr) {
    local_node_id_[trajectory_id] = insertion_result->node_id.node_index;
  }  
}

//added by skywalker
int MapBuilderBridge::GetNodeId(const int trajectory_id) {
  int node_id_in_traj;
  node_id_in_traj = local_node_id_.at(trajectory_id);
  
  return node_id_in_traj;
}
...
::cartographer::mapping::MapById<NodeId, TrajectoryNodePose> 
MapBuilderBridge::GetNodeListForFusion() {
  ::cartographer::mapping::MapById<NodeId, TrajectoryNodePose> node_poses_for_fuse = 
           map_builder_->pose_graph()->GetTrajectoryNodePoses();
  return node_poses_for_fuse;
}
