...
void CairoPaintSubmapSlices2( //from Skywalker, for submap output.
    int submap_index,
    const double scale,
    const std::map<::cartographer::mapping::SubmapId, SubmapSlice>& submaps,
    cairo_t* cr, std::function<void(const SubmapSlice&)> draw_callback) {
  ...
  //for (auto& pair : submaps) {
    //const auto& submap_slice = pair.second;
    int trajectory_index_diy = 0; //trajectory id is normally 0
    const ::cartographer::mapping::SubmapId id{trajectory_index_diy, submap_index};
    const auto& submap_slice = submaps.at(id);
    ...
    LOG(INFO) << "XXXXXXXXXXXXXXXXXXXXXXsubmap_slice.pose: " << submap_slice.pose; //Skywalker
    LOG(INFO) << "XXXXXXXXXXXXXXXXXXXXXXsubmap_slice.slice_pose: " << submap_slice.slice_pose; //Skywalker
    LOG(INFO) << "XXXXXXXXXXXXXXXXXXXXXXhomo: " << homo; //Skywalker
    
    LOG(INFO) << "XXXXXXXXXXXXXXXXXXXXXXsubmap_slice.width: " << submap_slice.width; //Skywalker
    LOG(INFO) << "XXXXXXXXXXXXXXXXXXXXXXsubmap_slice.height: " << submap_slice.height; //Skywalker    
...
PaintSubmapSlicesResult PaintSubmapSlices(
  ...
  LOG(INFO) << "box_size_x: " << bounding_box.sizes().x() + 2 * kPaddingPixel; //Skywalker
  LOG(INFO) << "box_size_y: " << bounding_box.sizes().y() + 2 * kPaddingPixel; //Skywalker
  LOG(INFO) << "origin_x: " << -bounding_box.min().x() + kPaddingPixel; //Skywalker
  LOG(INFO) << "origin_y: " << -bounding_box.min().y() + kPaddingPixel; //Skywalker
  LOG(INFO) << "box min x: " << bounding_box.min().x(); //Skywalker
  LOG(INFO) << "box min_y: " << bounding_box.min().y(); //Skywalker
...
PaintSubmapSlicesResult PaintSubmapSlices2( //created by Skywalker, for submap outtput.
    const std::map<::cartographer::mapping::SubmapId, SubmapSlice>& submaps,
    const double resolution,
    int submap_index) {
  Eigen::AlignedBox2f bounding_box;
      ...
      cairo_image_surface_create(kCairoFormat, size.x(), size.y()));
  {
    ...
    cairo_paint(cr.get());
    cairo_translate(cr.get(), origin.x(), origin.y());
    CairoPaintSubmapSlices2(submap_index, 1. / resolution, submaps, cr.get(),
                           [&cr](const SubmapSlice& submap_slice) {
                             cairo_set_source_surface(
                                 cr.get(), submap_slice.surface.get(), 0., 0.);
                             cairo_paint(cr.get());
