...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
  ...
  //LOG(INFO) << "?????????????????? Initialized Time: trajectory_connectivity_state_.LastConnectionTime " << last_connection_time; //added by Skywalker

  //LOG(INFO) << "?????????????????? node_id.trajectory_id == submap_id.trajectory_id: " << (node_id.trajectory_id == submap_id.trajectory_id)? 1 : 0  ; //added by Skywalker
  //LOG(INFO) << "?????????????????? global_localization_samplers_[node_id.trajectory_id]->Pulse(): " << global_localization_samplers_[node_id.trajectory_id]->Pulse(); //added by Skywalker
